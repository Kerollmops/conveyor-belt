...
use bevy::math::Vec3;
use bevy_rapier3d::dynamics::{RigidBody, RigidBodyHandle, RigidBodySet};
use bevy_rapier3d::geometry::{ColliderHandle, ColliderSet, Ray};
use bevy_rapier3d::math::{Point, Rotation, Vector};
use bevy_rapier3d::pipeline::{QueryFilter, QueryPipeline};
use bevy_rapier3d::utils::{WCross, WDot};
...
struct WheelDesc {
    ...
    pub chassis_connection_cs: Vec3,
...
pub struct Wheel {
    ...
    pub rotation: f32,
    delta_rotation: f32,
    roll_influence: f32, // TODO: make this public?
    /// The maximum force applied by the suspension.
    pub max_suspension_force: f32,
...
impl DynamicRayCastVehicleController {
    ...
    fn ray_cast(
        ...
        if let Some((collider_hit, mut hit)) = hit {
            if hit.toi == 0.0 {
                ...
                if let Some(hit2) =
                    collider.shape.cast_ray_and_get_normal(collider.position(), &up_ray, 1.0, false)
                {
            ...
            let proj_vel =
                wheel.raycast_info.contact_normal_ws.dot(&chassis_velocity_at_contact_point);
    ...
    pub fn update_vehicle(
        ...
        let chassis = bodies.get_mut_internal_with_modification_tracking(self.chassis).unwrap();
    ...
    fn update_friction(&mut self, bodies: &mut RigidBodySet, colliders: &ColliderSet, dt: f32) {
        let num_wheels = self.wheels.len();
...

        {
            for i in 0..num_wheels {
                ...
                if ground_object.is_some() {
                    ...
                    self.axle[i] = self.axle[i].try_normalize().unwrap_or_else(Vector::zeros);
            ...
            for wheel_id in 0..num_wheels {
                ...
                if ground_object.is_some() {
                    if wheel.engine_force != 0.0 {
                        ...
                        let contact_pt = WheelContactPoint::new(
                            &bodies[self.chassis],
                            ground_object.and_then(|h| colliders[h].parent()).map(|h| &bodies[h]),
                            wheel.raycast_info.contact_point_ws,
                ...
                if ground_object.is_some() {
                    ...
                    if impulse_squared > max_imp_squared {
                        ...
                        let factor = max_imp * bevy_rapier3d::utils::inv(impulse_squared.sqrt());
...
impl<'a> WheelContactPoint<'a> {
    pub fn new(
        ...
        fn impulse_denominator(body: &RigidBody, pos: &Vec3, n: &Vec3) -> f32 {
    ...
    pub fn calc_rolling_friction(&self, num_wheels_on_ground: usize) -> f32 {
        ...
        (-vrel * self.jac_diag_ab_inv / (num_wheels_on_ground as f32))
...
fn resolve_single_bilateral(
    ...
    let jac_diag_ab_inv = bevy_rapier3d::utils::inv(jac_diag_ab);
...
fn resolve_single_unilateral(body1: &RigidBody, pt1: &Vec3, normal: &Vec3) -> f32 {
    let vel1 = body1.velocity_at_point(pt1);
    let dvel = vel1;
    let dpt1 = pt1 - body1.center_of_mass();
    let aj = dpt1.gcross(*normal);
